...
func main() {
	fmt.Println(parse(part1))  // 4361
	fmt.Println(parse(input))  // 520019
	fmt.Println(parse2(part1)) // 4361
	fmt.Println(parse2(input)) // 520019
...
func parse(in string) int {
	...
	for i, line := range lines {
		// Replace . with _ cause \W ignores underscore.
		line = strings.ReplaceAll(line, ".", "_")
		var y = i

		// Find all the patterns and index.
		matches := re.FindAllStringIndex(line, -1)
		for _, match := range matches {
			val := line[match[0]:match[1]]
			n, err := strconv.Atoi(val)
			if err != nil {
				symbols = append(symbols, vec{
					x: match[0],
					y: y,
				})
			} else {
	...
	for i, num := range nums {
		for _, symbol := range symbols {
			for _, t := range translations {
				newX := symbol.x + t.x
				newY := symbol.y + t.y
				if (newX >= num.start && newX <= num.end-1) && newY == num.y {
					nums[i].marked = true
					break
				}
			}
	...
	for _, symbol := range symbols {
		...
		for _, num := range nums {
			for _, t := range translations {
				...
				newY := symbol.y + t.y
				if (newX >= num.start && newX <= num.end-1) && newY == num.y {
					adjacents = append(adjacents, num.n)
					break
				}
			}
		}
